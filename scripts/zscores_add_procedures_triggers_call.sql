...
SET i=0;
WHILE i < n DO
	SET pid = (SELECT pubId FROM publication LIMIT i,1);
    SET p_avg = (SELECT ROUND(AVG(review.score), 2) FROM review WHERE review.pubId = pid);
    SET p_stddev = (SELECT ROUND(STDDEV_POP(review.score), 2) FROM review WHERE review.pubId = pid);
    UPDATE publication SET publication.score_avg = p_avg WHERE publication.pubId = pid;
    UPDATE publication SET publication.score_stddev = p_stddev WHERE publication.pubId = pid;
    SELECT COUNT(*) FROM (SELECT gameId FROM review WHERE review.pubId = pid) AS temp INTO gn;
    SET j = 0;
    WHILE j < gn DO
        SET gid = (
            SELECT gameId FROM review WHERE review.pubId = pid
            LIMIT j,1
        );
        SET r_score = (SELECT score FROM review WHERE (review.pubId = pid) AND (review.gameId = gid));
        SET g_zscore = IF(p_stddev > 0, ROUND((r_score - p_avg) / p_stddev, 3), IF(r_score = p_avg, 0, NULL));
        INSERT INTO stat (gameId, pubId, zscore)
...
WHILE j < gn DO
	...
	SET r_score = (SELECT score FROM review WHERE (review.pubId = pid) AND (review.gameId = gid));
	SET g_zscore = IF(p_stddev > 0, ROUND((r_score - p_avg) / p_stddev, 3), IF(r_score = p_avg, 0, NULL));
	UPDATE stat SET stat.zscore = g_zscore WHERE (stat.pubId = pid) AND (stat.gameId = gid);
	SET j = j + 1;
END WHILE;
END;
//
...
CREATE TRIGGER review_insert_pubStatUpdate AFTER INSERT ON review
	FOR EACH ROW
    BEGIN
		INSERT INTO stat VALUES (NEW.gameId, NEW.pubId, NULL);
		CALL calc_stats_by_pub(NEW.pubId);
	END//
DELIMITER ;
...
CREATE TRIGGER review_beforeDelete_pubStatUpdate BEFORE DELETE ON review
	FOR EACH ROW
    BEGIN
		DELETE FROM stat WHERE stat.gameId = OLD.gameId AND stat.pubId = OLD.pubId;
	END//
DELIMITER ;
...
CREATE TRIGGER review_afterDelete_pubStatUpdate AFTER DELETE ON review
	FOR EACH ROW
    BEGIN
		CALL calc_stats_by_pub(OLD.pubId);
	END//
DELIMITER ;
